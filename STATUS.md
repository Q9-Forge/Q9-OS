...
| 🟡 | `0x2E` | F$VModul | Native header-parity/CRC validation, size bounds, return-buffer ABI, host tests, and live validation of `echo`/`csl` succeed; `/dd/CMDS/date` never reaches F$VModul because Microware F$Load already fails in the RBF/path-read step with `E$MNF` |
