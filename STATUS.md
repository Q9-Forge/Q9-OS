...
| 🟡 | `0x07` | F$Mem | Information request implemented and **fully wired**: handler `Q9K_SysFMem` in `q9kernel_entry.a` (reached through pointer cell `$1EB8`), registered in both dispatch tables. `d0=0` returns the data area size in `d0.l` and its upper bound in `a1`, read from the process descriptor's alloc base/size fields; every resize is refused with `E$NoRAM`, which is what the reference kernel does at `$61da` and what the manual mandates from V2.3 on. ABI verified twice over: manual pp. 465/466 and the disassembled original at module offset `$133C`. Host tests green, and a boot run with the handler in place reports `Vektor=0`. Still open: an emulator regression that actually *calls* `F$Mem` — the boot run only proves it breaks nothing |
