...
| Real driver module (OS-9, Init/Read/Write/GetStat/SetStat entry points reachable via `_os9_f_viread`) | ❌ Not started | Still the missing link, for **both** driver variants in this repo group: `dhf_manager.c` here calls the driver via `_os9_f_viread(pd->pd_dev, ...)`, i.e. a real OS-9 driver module with the standard jump table -- neither this repo's plain-C `driver/dhfdrv-68k.c` nor `Q9-DHFDRV-68k/driver/dhfdrv.c` (which already speaks the A0/A1/D0-D2 protocol the new emulator device expects, and is the natural template for this) are that module yet. Building one needs QCC's `driver` calling-convention/MODHEADER support, mid-flight on the `QCC-DEFMODUL` branch (not merged into `Q9-QCC` main, parser still incomplete as of 2026-09-25). Also open: this repo's manager protocol (`path_desc`/`_os9_f_viread`) and `Q9-DHFDRV-68k`'s driver protocol (A0/A1/D0-D2 registers) have never been reconciled -- whoever writes the real driver module needs to bridge the two, or `dhf_manager.c` needs adjusting to match. |
